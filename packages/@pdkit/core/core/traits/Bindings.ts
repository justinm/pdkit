...
export class Bindings implements IBindings {
  public static implement<T extends Construct>(
    instance: IBindings,
    callbacks?: { acceptBindingCallback?: AcceptBindingCallback; acceptChildCallback?: AcceptChildCallback }
  ) {
  ...
  private readonly acceptBindingCallback?: AcceptBindingCallback;
  private readonly acceptChildCallback?: AcceptChildCallback;
  private _index: number = 0;

  constructor(owner: Construct, callbacks?: { acceptBindingCallback?: AcceptBindingCallback; acceptChildCallback?: AcceptChildCallback }) {
    this.owner = owner;
    this.acceptBindingCallback = callbacks?.acceptBindingCallback;
    this.acceptChildCallback = callbacks?.acceptChildCallback;

    owner.node.scopes.forEach((scope) => {
      const callback = Bindings.tryOf(scope)?.acceptChildCallback;
  ...
  public bind(object: Construct, unique?: boolean) {
    ...
    if (this.acceptBindingCallback) {
      this.acceptBindingCallback(object);
    }
